...
public class SubscriberStompDoor implements MessageListener {
    ...
    public void onMessage(Message message, byte @Nullable [] pattern) {
        ...
        if (fullChannel.startsWith("DchatRoom:")) {
            // "chatRoom:123"에서 "123"만 추출하기
            String roomId = fullChannel.replace("DchatRoom:", "");
            // Redis에서 받은 byte[]를 문자열로 변환
            String publishMessage = new String(message.getBody());
            // STOMP로 클라이언트들에게 broadcasting
            messagingTemplate.convertAndSend("/chatRoom/" + roomId + "/door", publishMessage);
        }
    }
}